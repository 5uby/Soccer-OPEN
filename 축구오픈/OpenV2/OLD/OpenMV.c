...
char ReadCAM(unsigned char mode)
{
  ...
  {
    Serial1.write(0xAA);   //카메라에게 데이터 요청
    Serial1.write(mode);   //찾기 모드 
        
    startms = millis();
    timeout = 100;
    
    while(Serial1.available()<14 && millis() - startms < timeout)  ;
    ...
    if(l >= 14)
    {
      if(rcv[0] == 0xAA)
      {
        ...
        if((chksum & 0x7F) == 0) //정상적인 데이터
        {
          ...
          if(OBJ[0].x == 0 && OBJ[0].y == 0)  find_fail(0);
          else  find_success(0);
